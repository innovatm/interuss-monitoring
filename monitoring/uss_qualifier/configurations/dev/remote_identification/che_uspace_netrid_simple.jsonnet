// This file is a top-level test configuration for `che_uspace_netrid`.

// Regardless of environment (which systems are tested and where they are), this baseline defines the overall behavior
// of the test.
local baseline = import 'che_uspace_netrid_lib/baseline_simple.libsonnet';
...
// configuration.
// Note that the environment does not need to be defined in a separate file (che_uspace_netrid.libsonnet).
...
local env = import 'che_uspace_netrid_lib/local/env_all.libsonnet';
