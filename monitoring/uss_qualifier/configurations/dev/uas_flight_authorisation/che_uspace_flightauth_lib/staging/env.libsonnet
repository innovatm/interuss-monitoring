function(participants) {
  // This file contains environmental (non-baseline) parameters for the che_uspace_flightauth.jsonnet test configuration for the staging environment.
  // It is parameterized on which participants to include in the test.  Each participant is expected to have a "staging_env" block in their definitions.
  // Top-level keys are used in che_uspace_flightauth.jsonnet when this content is provided as `env`.
  resource_declarations: {
    ...
    test_env_version_providers: {
      ...
      specification: {
        instances: [
          participant.staging_env.test_env_version_provider
          for participant in participants
          if 'test_env_version_provider' in participant.staging_env
        ],
      },
    },
    ...
    prod_env_version_providers: {
      ...
      specification: {
        instances: [
          participant.staging_env.prod_env_version_provider
          for participant in participants
          if 'prod_env_version_provider' in participant.staging_env
        ],
      },
    },
    ...
    flight_planners: {
      ...
      specification: {
        flight_planners: [
          participant.staging_env.flight_planner
          for participant in participants
          if 'flight_planner' in participant.staging_env
        ],
      },
    },
    ...
    mock_uss: {
      ...
      specification: {
        ...
        mock_uss_base_url: 'https://mock.dss.uspacekeeper.com/mock_uss_scdsc_interaction_log',
      },
    },
